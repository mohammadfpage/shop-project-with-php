...
if(isset($_POST['sub1'])){
    ...
    $result = $conn->prepare("UPDATE users SET username=?, email=? , password=?, Access=? WHERE id=?");
    $result->bind_param("ssssi", $username, $email, $password, $Access, $getid);
    $result->execute();
  }
?>
          ...
          <ul class="list-unstyled">
            ...
            <li x-data="dropdown" class="sidebar-item">
              ...
              <ul x-show="open" x-transition class="submenu">
                ...
                <li class="submenu-item">
                  <a href="edituser.php">ویرایش کاربران</a>
                </li>
              </ul>
            </li>
            ...
            <?php foreach ($users as $user): ?>
              ...
              <div class="col-md-6">
                  <label for="email" class="text-gray-600 fw-bold"
                    >نام کاربری</label
                  >
                  <input name="username" id="username" type="text" class="form-control mt-2" value="<?= $user['username']; ?>"/>
                </div>
                ...
                <div class="col-md-6">
                  <label for="email" class="text-gray-600 fw-bold"
                    >ایمیل</label
                  >
                  <input name="email" id="email" type="text" class="form-control mt-2" value="<?= $user['email']; ?>"/>
                </div>
                <div class="col-md-6">
                  <label for="password" class="text-gray-600 fw-bold"
                    > رمز عبور</label>
                  
                  <input name="password" id="password" type="text" class="form-control mt-2" value="<?= $user['password']; ?>"/>
                </div>
              </div>
              <div class="row mt-2">
                

              <select class="form-select  mb-3 m-auto mt-4" aria-label="Large select example" name="who" style="margin:auto;">
            <option  selected class="m-auto"><?php  
                if($user["Access"]==1)
                    echo "admin";
                else
                    echo "user";
